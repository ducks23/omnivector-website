...
class ServerlessPlugin {
  constructor(serverless, options) {
    ...
    this.commands = {
      ...
      s3DomainInfo: {
        usage: 'Fetches and prints out the s3 website FQDN',
        lifecycleEvents: [
          's3DomainInfo',
        ],
      },
  ...
  async s3DomainInfo() {
    ...
    const outputs = result.Stacks[0].Outputs;
    const output = outputs.find(
      entry => entry.OutputKey === 'WebAppS3BucketUrl',
    );

    if (output && output.OutputValue) {
      this.serverless.cli.log(`S3 FQDN: ${output.OutputValue}`);
      return output.OutputValue;
    }

    this.serverless.cli.log('S3 FQDN Not Found');
    const error = new Error('Could not extract S3 FQDN');
    throw error;
  }
